...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot DEX SNR from GWF frame files")
...
def make_plots(args, dex_snr):
    ...
    ax = axes[0]
    ax.scatter(segment_index, np.array(
        dex_snr['network_snr']), label='Network statistic (Excess power)', s=10, color='salmon')
    ax.axhline(y=8, color='black', ls='--')
    ax.scatter(segment_index, np.array(dex_snr['combined_statistic']),
               label='Combined statistic (Coherent excess power)', s=10, marker='x',
               color='black')

    ax = axes[1]
    if 'null_stream' in dex_snr:
        ax.scatter(segment_index, np.array(
            dex_snr['null_stream']), label='Null stream statistic', s=5)
    elif 'mismatch_overlap' in dex_snr:
        ax.set_ylim(-0.1, 1.1)
        ax.set_ylabel("Overlap")
        ax.scatter(segment_index, np.array(
            dex_snr['mismatch_overlap']), label='Overlap statistic', s=5)
    else:
        raise ValueError(
            "Neither 'null_stream' nor 'mismatch_overlap' found in data")
    ax.axhline(y=8, color='black', ls='--')
